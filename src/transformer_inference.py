...
def main():
    ...
    for n, (_, eval_idxs) in enumerate(
        ...
        pipe = pipeline(task="sentiment-analysis", model=fold_model_path, tokenizer=tokenizer,)
        infer_ds = eval_ds.map(infer(pipe, **TOKENIZER_KWARGS)) #.datasets.Value("float64")
        y_true = infer_ds["label"]
        y_pred = [CLASS_LABELS.str2int(x[0]["label"]) for x in infer_ds["inferred_label"]]
        y_true_pooled.extend(y_true)
        y_pred_pooled.extend(y_pred)
        print(classification_report(y_true, y_pred, labels=range(CLASS_LABELS.num_classes), target_names=CLASS_NAMES,
                                    zero_division=0))
        print("\n")

    print(f"#### POOLED SCORES ####")
    print(classification_report(y_true_pooled, y_pred_pooled, labels=range(CLASS_LABELS.num_classes),
